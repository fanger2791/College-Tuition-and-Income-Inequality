...
file_path <- "/Users/michaelfang/Downloads/SCF_AER_submit.xlsx" # Update with the actual path to your Excel file

# Read the "Data for Figure 9" sheet from the Excel file, starting from the second row
data <- read_excel(file_path, sheet = "Data for Figure 9", skip = 4)


# Reshape the data: create two columns for grid log income and EMG (ML) PDF, with corresponding years
data_long <- data %>%
  pivot_longer(
    cols = c(`...1`, `...3`),
    names_to = "Year",
...
ggplot(data_long, aes(x = GridLogIncome, y = EMG_ML_PDF, color = Year)) +
  geom_line() +
  labs(title = "Comparison of Income Distribution for 1989 and 2016",
